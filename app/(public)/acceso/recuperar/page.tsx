...
import { forgotPasswordAction } from "@/app/(public)/acceso/actions";
...
export default function RecuperarPage({ searchParams }: RecuperarPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md px-6 py-8 sm:py-14 sm:px-10">
      ...
      {!successMessage ? (
        <form action={forgotPasswordAction} className="mt-8 rounded-2xl border border-cordero bg-cordero-card p-6">
          ...
          <button
            ...
            type="submit"
          >
            {COPY.auth.forgotPasswordButton}
          </button>
        </form>
      ) : null}
